...
void Parser::Op() {
		if (la->kind == 4 /* "+" */) {
			Get();
			stackOp.push_back({PLS, L""}); PK++;
		} else if (la->kind == 5 /* "-" */) {
			Get();
			stackOp.push_back({MNS, L""}); PK++;
		} else if (la->kind == 6 /* "*" */) {
			Get();
			stackOp.push_back({MUL, L""}); PK++;
		} else if (la->kind == 7 /* "/" */) {
			Get();
			stackOp.push_back({DIV, L""}); PK++;
		} else SynErr(43);
}
...
void Parser::RelOp() {
		if (la->kind == 8 /* "==" */) {
			Get();
			stackOp.push_back({EQU, L""}); PK++;
		} else if (la->kind == 9 /* "<" */) {
			Get();
			stackOp.push_back({LSS, L""}); PK++;
		} else if (la->kind == 10 /* ">" */) {
			Get();
			stackOp.push_back({GTR, L""}); PK++;
		} else if (la->kind == 11 /* "><" */) {
			Get();
			stackOp.push_back({NEQ, L""}); PK++;
		} else SynErr(44);
}
...
void Parser::LogicOp() {
		if (la->kind == 12 /* "||" */) {
			Get();
			stackOp.push_back({OR, L""}); PK++;
		} else if (la->kind == 13 /* "&" */) {
			Get();
			stackOp.push_back({AND, L""}); PK++;
		} else SynErr(45);
}
...
void Parser::P_varId(bool set) {
		...
		P_var(pname);
		Expect(20 /* ")" */);
		(set)? stackOp.push_back({SET_PID, L""}),PK++ : stackOp.push_back({GET_PID, L""}),PK++;
		stackOp.push_back({NL, wstring(pname)});
